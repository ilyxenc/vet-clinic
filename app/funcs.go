...
// пустой fiber.Map{} передаётся из-за требования функции c.Render к передаче 3-ёх параметров
func createAnimalGet(c *fiber.Ctx) error {
...
func createAnimal(c *fiber.Ctx) error {
	// Объявление нового элемента Animal
	var item Animal
...
func readAnimal(c *fiber.Ctx) error {
...
func updateAnimalGet(c *fiber.Ctx) error {
	// Получение данных о животном по конкретному id
...
func deleteAnimal(c *fiber.Ctx) error {
	...
	data = removeAnimalFromSlice(data, c.Params("id"))

	// Преобразование слайса из структур Animal в набор байт, возвращение статуса ошибки в случае, если она есть
